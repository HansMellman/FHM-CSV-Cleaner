...
st.set_page_config(
    page_title='Player Ratings Cleaner',
    page_icon="📄"
)
...
player_ids = st.file_uploader("Please select the 'player_skater_stats_rs_CONVERTED.csv' CSV that you have just processed, this is necessary for the 'Player Ratings' calculations.")

if player_ratings is not None and player_ids is not None:
    if player_ratings.name != 'player_ratings.csv' or player_ids.name != 'player_skater_stats_rs_CONVERTED.csv':
        st.write("Please upload the correct player_ratings.csv and player_skater_stats_rs_CONVERTED.csv")
    elif player_ratings.name == 'player_ratings.csv' and player_ids.name == 'player_skater_stats_rs_CONVERTED.csv':
        player_ratings = pd.read_csv(player_ratings, sep=';')
        player_ids = pd.read_csv(player_ids,
                                 usecols=[0, 1],
                                 )
        ...
        nhl_player_ratings = player_ratings[player_ratings['PlayerId'].isin(player_ids['PlayerId'])]
